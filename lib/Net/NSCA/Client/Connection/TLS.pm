...
###############################################################################
# CONSTANTS
Readonly our $ENCRYPTION_TYPE_NONE => 'none';
Readonly our $ENCRYPTION_TYPE_XOR  => 'xor';
...
sub encrypt {
	...
	if ($self->encryption_type == $ENCRYPTION_TYPE_NONE) {
		# No encryption, so return the same 
	}
	elsif ($self->encryption_type == $ENCRYPTION_TYPE_XOR) {
